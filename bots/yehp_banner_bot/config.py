...
def _normalize_schedule_time(value: str, env_name: str) -> str:
    ...
    if len(parts) not in {1, 2}:
        raise ConfigError(f"{env_name} entries must use HH:MM, e.g. 12:00,18:00")
...
class BotConfig:
    ...
    def from_env(cls, env_file: Path | None = None) -> "BotConfig":
        ...
        output_dir = _env_path(
            "BOT_OUTPUT_DIR",
            Path("bots/yehp_banner_bot/output"),
        )
        ...
        return cls(
            brand_name=os.getenv("BOT_BRAND_NAME", "YEHP"),
            brand_tagline=os.getenv(
                "BOT_BRAND_TAGLINE",
                "Doctor-guided herbal wellness",
            ),
            logo_url=os.getenv(
                "BOT_LOGO_URL",
                "",
            ...
            reel_audio_file=_env_path(
                "BOT_REEL_AUDIO_FILE",
                Path("bots/yehp_banner_bot/bombinsound-trending-instagram-reels-music-499599.mp3"),
            ),
    ...
    def missing_publish_settings(self) -> list[str]:
        ...
        if self.publish_media_type == "reel" and not self.reel_audio_file.exists():
            missing.append("BOT_REEL_AUDIO_FILE")
        return missing
