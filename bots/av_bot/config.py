...
class BotConfig:
    ...
    def from_env(cls, env_file: Path | None = None) -> "BotConfig":
        ...
        output_dir = _env_path("BOT_OUTPUT_DIR", Path("bots/av_bot/output"))
        ...
        return cls(
            ...
            tutorlix_site_base_url=os.getenv(
                "TUTORLIX_SITE_BASE_URL",
                "https://tutorlix.com",
            ).rstrip("/"),
            profile_type=os.getenv("BOT_PROFILE_TYPE", "professional").strip()
            or "professional",
            note_page_size=_env_int("BOT_NOTE_PAGE_SIZE", 2000),
            note_fetch_max_pages=_env_int("BOT_NOTE_FETCH_MAX_PAGES", 5),
            recent_history_limit=_env_int("BOT_RECENT_HISTORY_LIMIT", 25),
