...
LOGGER = logging.getLogger("linkedin-banner-bot")
...
def run_loop(config: BotConfig) -> None:
    ...
    LOGGER.info(
        "Starting LinkedIn banner bot; interval=%ss dry_run=%s",
...
def _run_scheduled_loop(config: BotConfig) -> None:
    ...
    LOGGER.info(
        "Starting LinkedIn banner bot; schedule=%s timezone=%s run_on_start=%s dry_run=%s",
...
def run_once(config: BotConfig) -> RunResult:
    ...
    try:
        results = publisher.publish_image_posts(image_url=image_url, text=text)
    except Exception:
    ...
    store.mark_used(content_index, image_path, buffer_post_ids=post_ids)
    LOGGER.info(
        "Published LinkedIn image through Buffer post_ids=%s image_url=%s",
        ",".join(post_ids),
        image_url,
    )
    return RunResult(
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate branded banners and publish them to LinkedIn through Buffer.",
    )
