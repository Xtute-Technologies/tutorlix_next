...
mutation CreateLinkedInImagePost($input: CreatePostInput!) {
...
class BufferPublisher:
    ...
    def publish_image_post(
        ...
        *,
        ...
        payload = {
            ...
            "mode": self.post_mode,
            "source": "tutorlix-linkedin-banner-bot",
            "assets": {
                "images": [
                    {
                        "url": image_url,
    ...
    def _preflight_image_url(self, image_url: str) -> None:
        if not image_url:
            raise BufferPublishError("Missing public image URL")

        LOGGER.info("Preflight public image URL for Buffer: %s", image_url)
        try:
            ...
            raise BufferPublishError(
                f"Public image URL is not reachable from the bot: {exc}"
            ) from exc

        content_type = response.headers.get("Content-Type", "")
        content_length = response.headers.get("Content-Length", "")
        LOGGER.info(
            "Public image URL preflight status=%s content_type=%s content_length=%s",
        ...
        if response.status_code >= 400:
            raise BufferPublishError(
                f"Public image URL returned HTTP {response.status_code}"
            )
