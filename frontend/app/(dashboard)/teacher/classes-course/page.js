...
const classSchema = z.object({
    ...
    link: z.union([z.string().url('Must be a valid URL'), z.literal(''), z.undefined()]).optional(),
    // teacher field removed as it's implicit
...
export default function TeacherCourseClassesPage() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [classesData, productsData] = await Promise.all([
                courseClassAPI.getAll(),
                productAPI.getAll({ my_products: 'true' }), // Fetch only my products
            ]);
    ...
    const columns = [
        ...
        // Teacher column removed as it's the current user
        {
            accessorKey: 'meeting_url',
            header: 'Meeting',
            cell: ({ row }) => (
                <Button asChild variant="outline" size="sm">
                    <a href={row.original.meeting_url || row.original.link || '#'}>
    ...
    return (
            <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">My Course Classes</h1>
                        <p className="text-gray-600 mt-1">
                            Manage classes for your courses
                        </p>
                    </div>
                    <Button onClick={() => {
                        setEditingClass(null);
