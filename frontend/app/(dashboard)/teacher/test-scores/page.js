...
export default function TeacherTestScoresPage() {
    ...
    const fetchData = async () => {
        try {
            ...
            const [scoresData, studentsData] = await Promise.all([
                testScoreAPI.getAll(), // Filters by teacher in backend
                authService.getAllUsers({ role: 'student' }), // Fetches all students
            ]);
    ...
    const handleSubmit = async (data) => {
        try {
            ...
            setMessage({
                type: 'error',
                text: error.response?.data?.student ? `Student Error: ${error.response.data.student}` :
                      (error.response?.data?.detail || 'Failed to save test score'),
            });
        } finally {
            setSubmitting(false);
        }
    ...
    return (
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold">My Students' Scores</h1>
                        <p className="text-gray-600 mt-1">Manage test scores for your students</p>
                    </div>
                    <Button onClick={() => {
                        setEditingScore(null);
                        setShowForm(true);
                        ...
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                {editingScore
                                    ...
                                    : 'Record a new test score for a student. Note: Student must be enrolled in your course.'}
                            </DialogDescription>
                        </DialogHeader>
                        <FormBuilder
