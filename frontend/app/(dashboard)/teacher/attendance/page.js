  ...
  const fetchData = async () => {
    try {
      ...
      const [attendanceData, studentsData] = await Promise.all([
        ...
        authService.getAllUsers({ role: 'student' }), // Fetches all students, backend validates if they are in teacher's course on save
      ]);
    ...
    try {
      ...
      setMessage({
        ...
        text: error.response?.data?.student ? `Student Error: ${error.response.data.student}` : 
              (error.response?.data?.detail || 'Failed to save attendance. Ensure students are enrolled in your courses.'),
      });
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage attendance for your students</p>
          </div>
          <Button onClick={() => {
            resetForm();
            setShowForm(true);
        ...
        <Dialog open={showForm} onOpenChange={setShowForm}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Select students and mark their attendance status. Note: You can only mark attendance for students enrolled in your courses.
              </DialogDescription>
            </DialogHeader>
